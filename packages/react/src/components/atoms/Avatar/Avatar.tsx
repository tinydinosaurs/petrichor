...
export const Avatar = React.forwardRef<HTMLSpanElement, AvatarProps>(
  ({ src, alt, name, size = 'md', status, asChild = false, className, children, ...rest }, ref) => {
    ...
    return (
      <Comp ref={ref} className={classes} {...rest} aria-label={ariaLabel}>
        {asChild ? (
          children
        ) : (
          <>
            ...
            {showIcon && (
              <User size={iconSize} weight="regular" aria-hidden="true" className={styles.icon} />
            )}

            {status && (
              <span
                className={[styles.status, statusClass[status]].join(' ')}
                role="status"
