...
export const StoryCard = React.forwardRef<HTMLElement, StoryCardProps>(
	(
		...
		const ctaLabel = (() => {
			if (mode === 'anonymous' || mode === 'completed')
				return 'Read story';
			if (
		...
		const segmentLabel = (() => {
			...
			if (maxSegments)
				return `${currentSegments} / ${maxSegments} segments`;
				...
				<Card.Body>
					...
					{mode === 'active' && (
						<Text variant="body-sm" className={styles.teaser}>
							Pick up where the last writer left off — you'll see
							their final sentence when you join.
						</Text>
					)}
				...
				<Card.Footer className={styles.footer}>
					<div className={styles.footerMeta}>
						...
						<div className={styles.meta}>
							{segmentLabel && (
								<Text variant="caption">{segmentLabel}</Text>
							)}
