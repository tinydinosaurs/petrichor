...
const TokenTag = ({ label, value }: { label: string; value: string }) => (
	...
	>
		<span style={{ ...MONO, color: 'var(--ptr-color-text-tertiary)' }}>
			{label}
		</span>
		<code
			style={{
	...
	return (
		...
		>
			...
			>
				<span
					style={{
						...
						fontWeight: 600,
						fontFamily:
							'var(--ptr-typography-font-mono, monospace)',
						color: 'var(--ptr-color-text-primary)',
			...
			>
				<TokenTag
					label="font"
					value={`--ptr-typography-font-${tokens.font}`}
				/>
				<TokenTag
					label="size"
					value={`--ptr-typography-size-${tokens.size}`}
				/>
				<TokenTag
					label="weight"
					value={`--ptr-typography-weight-${tokens.weight}`}
				/>
				<TokenTag
					label="line-height"
					value={`--ptr-typography-line-height-${tokens.lineHeight}`}
				/>
