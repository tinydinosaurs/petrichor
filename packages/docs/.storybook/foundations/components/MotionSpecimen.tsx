    ...
    <div style={{ marginBottom: '24px' }}>
      <button
        ...
        style={{
          padding: '8px 16px',
          marginBottom: '16px',
          background: isPlaying ? 'var(--ptr-color-bg-elevated)' : 'var(--ptr-color-brand-primary)',
          color: isPlaying ? 'var(--ptr-color-text-secondary)' : 'var(--ptr-color-text-on-brand)',
          border: 'none',
          ...
          >
            <div
              style={{
                ...
                borderRadius: '4px',
                transition: phase === 'animate' ? `width var(${token}) ease-out` : 'none',
              }}
            />
          </div>
...
export const EasingCurve = ({ token, label, value, description }: EasingCurveProps) => {
  const [phase, setPhase] = useState<'idle' | 'reset' | 'animate'>('idle');
      ...
      >
        <div
          style={{
            ...
            borderRadius: '50%',
            background: 'var(--ptr-color-brand-primary)',
            transition: phase === 'animate' ? `left 600ms var(${token})` : 'none',
          }}
        />
      </div>
      <div style={s.label}>{label}</div>
      <code style={{ ...s.mono, display: 'block', marginTop: '4px' }}>{value}</code>
      <div style={{ ...s.description, marginTop: '8px' }}>{description}</div>
    </div>
  );
};
...
export const TransitionDemo = ({ token, label, description }: TransitionDemoProps) => {
    ...
    >
      <div
        style={{
          width: '120px',
          height: '60px',
          background: active ? 'var(--ptr-color-brand-primary)' : 'var(--ptr-color-bg-surface)',
      ...
      >
        <span
          style={{
            ...s.label,
            color: active ? 'var(--ptr-color-text-on-brand)' : 'var(--ptr-color-text-secondary)',
            transition: `var(${token})`,
          }}
        >
          Hover me
...
export const MotionPrinciple = ({ title, description, good, bad }: MotionPrincipleProps) => (
  <div
    style={{
      padding: '16px',
      background: 'var(--ptr-color-bg-surface)',
  ...
  >
    ...
    <div style={{ display: 'flex', gap: '16px', fontSize: '12px' }}>
      <div style={{ flex: 1 }}>
        ...
        <div style={{ color: 'var(--ptr-color-text-secondary)' }}>{good}</div>
      </div>
      <div style={{ flex: 1 }}>
        <div
          style={{
        ...
        <div style={{ color: 'var(--ptr-color-text-secondary)' }}>{bad}</div>
      </div>
    </div>
  </div>
);
