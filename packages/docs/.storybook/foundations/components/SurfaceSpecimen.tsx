  ...
  >
    {tokens.map(({ token, label, description }) => (
      <div key={token}>
        ...
        >
          <span style={{ ...s.label, color: 'var(--ptr-color-text-secondary)' }}>{label}</span>
        </div>
        <code style={{ ...s.mono, display: 'block', marginBottom: '4px' }}>{token}</code>
  ...
  <div style={{ marginBottom: '24px' }}>
    {tokens.map(({ token, label, value }) => (
      ...
      >
        ...
        />
        <div>
          <code style={{ ...s.mono, display: 'block', marginBottom: '2px' }}>{token}</code>
          <div style={s.label}>
